...
instance Show a => Show (AMatroid a) where
  show x = Data.Maybe.fromMaybe defShow $ w_showName x
...
a_restriction m x0 = a_namedRestriction name m x0
            where e = x0 `S.intersection` w_groundset m
                  ...
                  m_name = Data.Maybe.fromMaybe "M" $ w_showName m
...
a_namedRestriction name m x0 = wrappedMatroid {
  ...
  , w_loops = S.intersection e <$> w_loops m
  , w_coRk = (`D.coRk` e) <$> w_rk m
  , w_coloops = (`D.coloops` e) <$> w_rk m
...
a_namedContraction name m x0 = wrappedMatroid {
  ...
  , w_rk =    new_rk
  , w_indep = new_indep
  , w_basis = D.basis <$> new_indep
  , w_cl =    (\ cl_ x -> (cl_ (x `S.union` t)) `S.intersection` e) <$> w_cl m
  , w_loops = (\cl_ -> cl_ t `S.intersection` e) <$> w_cl m
  , w_coRk =  (`D.coRk` e) <$> new_rk
  , w_coloops = S.intersection e <$> w_coloops m
  , w_contraction = w_contraction m -- (M.X0).X1 = M.X1 whenever defined.
        ...
        new_rk = (\ rk_ rt_ x -> rk_ (x `S.union` t) - rt_ ) <$> w_rk m    <*> rt
        new_indep = (\ indep_ bt_ x -> indep_ (x `S.union` bt_)) <$> w_indep m <*> bt
...
a_namedDual name m = wrappedMatroid {
  ...
  , w_dual = Just m -- bounce back to the original matroid
        ...
        new_rk = w_coRk m `mplus` ((`D.coRk` e) <$> w_rk m)
        new_indep = D.indep <$> new_rk
